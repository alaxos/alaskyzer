		...
		<div class="panel-body">
			...
			<?php foreach ($technologies as $i => $technology): ?>
				<tr>
					<td>
						<?php
						echo $this->AlaxosForm->checkBox('Technology.' . $i . '.id', array('value' => $technology->id, 'class' => 'model_id'));
						?>
					</td>
					<td>
						<?php echo h($technology->name) ?>
					</td>
					<td>
						<?php echo h($technology->to_display_timezone('created')); ?>
					</td>
					<td>
...
// 						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span> ' . __d('alaxos', 'delete'), ['action' => 'delete', $technology->id], ['confirm' => __('Are you sure you want to delete # {0}?', $technology->id), 'escape' => false]);
						...
						echo '&nbsp;&nbsp;';
						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span>', ['action' => 'delete', $technology->id], ['confirm' => __('Are you sure you want to delete # {0}?', $technology->id), 'escape' => false]);
						?>
					</td>
				</tr>
			<?php endforeach; ?>
